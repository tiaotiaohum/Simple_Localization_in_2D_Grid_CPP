...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	vector< vector <float> > newGrid;
  	...
  	vector<float> new_rowin (width, belief_per_cell);
  	for (int iin = 0; iin < height; iin++ ){
    	newGrid.push_back(new_rowin); 
    }
  ...
  float blurring) 
{
  ...
  // your code here
  int h_mo = beliefs.size();
  int w_mo = beliefs[0].size();
  vector<float> new_row_mo (w_mo, 0);
  vector < vector <float> > newGrid (h_mo, vector <float>(w_mo, 0));  
  
  for (int i_mo=0; i_mo<h_mo; i_mo++){
  	for (int j_mo=0; j_mo<w_mo; j_mo++){
    	int new_i = (i_mo+dy)%h_mo;
      	int new_j = (j_mo+dx)%w_mo;
      	newGrid[new_i][new_j] = beliefs[i_mo][j_mo];
    }
...
{	
	vector< vector <float> > newGrid;
  	...
  	bool hit;
	// your code here
  	int h_se = beliefs.size();
  	int w_se = beliefs[0].size();
   	vector<float> row_se (w_se, 0);
  
  	for (int ise=0; ise<h_se; ise++){
      	row_se.clear();
    	for (int jse=0; jse<w_se; jse++){
        	hit = color == grid[ise][jse];
          	row_se.push_back(beliefs[ise][jse]*(hit*p_hit+(1-hit)*p_miss));
        }
      	newGrid.push_back(row_se);
    }
